...
class Detection:
    ...
    @classmethod
    def from_results(cls, pred: np.ndarray, names: Dict[int, str]) -> List[Detections]:
        result = []
...
def draw_boxes(img, bbox, identities=None, categories=None, confidences = None, names=None, colors = None):
    for i, box in enumerate(bbox):
        ...
        id = identities[i] if identities is not None else 0
            ...
            if len(det):
                ...
                if opt.track:
                    ...
                    tracks = byte_tracker.update(
                                            output_results=detections2boxes(detections=Detection.from_results(det.cpu().numpy(), names = names)),
                                            img_info= im0.shape,
                                            img_size= im0.shape
                                        )
                    
                    tracked_dets = match_detections_with_tracks(detections=Detection.from_results(det.cpu().numpy(), names = names), tracks=tracks)
                    ...
                    for detection in tracked_dets:
                        ...
                        bbox_xyxy_result.append(bbox)

                        tracker_id = detection.tracker_id
                        identities_result.append(tracker_id)

                        class_name = detection.class_id
                        categories_result.append(class_name)

                    # draw boxes for visualization
                    if len(tracked_dets)>0:
                        print(tracked_dets)
                        bbox_xyxy = bbox_xyxy_result
                        ...
                        if opt.show_track:
                            ...
                            for t, track in enumerate(tracks):
                                ...
                                track_color = colors[int(track.detclass)] if not opt.unique_track_color else byte_tracker.color_list[t]
